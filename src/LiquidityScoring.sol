...
contract LiquidityScoring is ILiquidityScoring, Ownable, Pausable {
    ...
    function calculateLiquidityScore(
        ...
        uint256 totalScore = (
            (marketCapScore * MARKET_CAP_WEIGHT) + (distributionScore * DISTRIBUTION_WEIGHT)
...
external view override returns (uint256)
     ...
     */
    function updateTokenInfo(address token, uint256 marketCap, uint256 dailyVolume) external onlyOwner {
        require(token != address(0), "Invalid token");
        require(marketCap > 0, "Invalid market cap");

        tokenInfo[token] = TokenInfo({marketCap: marketCap, dailyVolume: dailyVolume, lastUpdate: block.timestamp});
