...
interface IRiskAnalyzerHook {
     ...
     */
    function getPoolRiskMetrics(bytes32 poolId) 
        external 
        view 
     ...
     */
    function getPositionRiskScore(address user, bytes32 poolId)
        external
        view
        returns (uint256);
...
interface IRiskNotifier {
     ...
     */
    function getUserNotifications(address user)
        external
        view
...
interface IRiskRegistry {
     ...
     */
    function getPoolParameters(bytes32 poolId)
        external
        view
...
interface IUniswapV4RiskAnalyzerHook {
     ...
     */
    function getRiskParameters(bytes32 poolId)
        external
        view
        returns (RiskParameters memory);
