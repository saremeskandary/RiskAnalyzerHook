...
contract RiskAggregator is IRiskAggregator, Ownable, Pausable {
    ...
    constructor(
        ...
        if (_volatilityOracle == address(0) || _liquidityScoring == address(0) ||
            _positionManager == address(0) || _riskRegistry == address(0)) 
     ...
     */
    function aggregatePoolRisk(bytes32 poolId)
        external
        override
        whenNotPaused
        returns (uint256 totalRiskScore)
    {
        ...
        IVolatilityOracle.VolatilityData memory volData = 
            volatilityOracle.getVolatilityData(poolId);
     ...
     */
    function aggregateUserRisk(address user)
        external
        override
        whenNotPaused
        returns (uint256 totalRiskScore)
    {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            IPositionManager.PositionData memory position = 
                positionManager.getPositionData(user, pools[i]);
        ...
        )
    {
        ...
        averageRisk = systemMetrics.riskCount > 0 ?
            totalRisk / systemMetrics.riskCount : 0;
    ...
    function _calculateLiquidityScore(
        ...
        IRiskRegistry.RiskParameters memory params = 
            riskRegistry.getPoolParameters(poolId);
            
        uint256 liquidityThreshold = params.liquidityThreshold;
        if (liquidityThreshold == 0) return MAX_RISK_SCORE;
        
        uint256 currentLiquidity = liquidityScoring.calculateStabilityScore(
            poolId,
            liquidityThreshold
        );
        
        return currentLiquidity >= liquidityThreshold ?
            0 : ((liquidityThreshold - currentLiquidity) * MAX_RISK_SCORE) / liquidityThreshold;
    }

    /**
     * @notice Calculate position score
     */
    function _calculatePositionScore(
        bytes32 poolId
    ) internal view returns (uint256) {
        IPositionManager.PositionData memory position = 
            positionManager.getPositionData(address(this), poolId);
